...
namespace Genetic_AI
{
    public partial class MainWindow : Window
    {
        ...
        Rectangle Create_Rectangle(double desirection_x, double desirection_y, int color, int target)
        {
            int bytecolor;
            ...
            if (target == 0)
            {
                ...
                betweensize = desirection_x * 1000 * length_ratio;
                vertical_gap = desirection_y * 1500 * depth_ratio + Canvas1.Margin.Bottom;
            }
            ...
            else
            {
                bytecolor = (color*1515) % 255;
                betweensize = desirection_x * 1000 * length_ratio;
                vertical_gap = desirection_y-150;
            }

            Rectangle rectangle = new Rectangle
            {
                Width = 600 * length_ratio,
                Height = 600 * depth_ratio,
                Fill = new SolidColorBrush(Color.FromRgb((byte)bytecolor, 100 ,100))
            };
                    ...
                    =>
                    {
                        foreach (int[] gene in genetic_Main_Engine.gene_set)
                        {
                            ...
                            =>
                            {
                                ...
                                textbox_GeneList.AppendText($"index{index} Score: "
                                    + genetic_Main_Engine.Score_Calculation(gene, genetic_Main_Engine.genetic_target_data).ToString() + "\n");
